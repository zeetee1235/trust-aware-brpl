...
set_root_address_and_prefix(void)
{
  ...
  /* Advertise autonomous (SLAAC) but NOT on-link.
   * On-link causes root to resolve all aaaa::/64 addresses via NDP,
   * bypassing the RPL routing table for multi-hop sensor replies.
   * Without ONLINK, root uses RPL routes for all downward traffic. */
