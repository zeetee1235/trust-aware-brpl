...
\koabstract{%
저전력 손실 네트워크(LLN)에서 RPL은 낮은 제어 오버헤드와 안정적 부모 유지가 중요하지만,
바로 그 안정성이 sinkhole 공격자에게는 유리하게 작용할 수 있다.
공격자는 허위로 낮은 rank를 광고해 먼저 트래픽을 자신에게 집중시키고
(\emph{경로 장악, route capture}),
이후 선택적 포워딩이나 grayhole 동작으로 가용성을 저하시킨다.
기존 PDR 중심 평가는 이 선행 장악 단계를 놓치기 쉽고,
신뢰 기반 즉시 차단은 반대로 부모 교체(churn)를 폭증시켜
보안과 안정성 사이의 trade-off를 악화시킨다.
또한 BRPL류의 queue-aware 라우팅은 혼잡 우회에는 강하지만,
공격자가 큐 광고를 조작하거나 초기에는 정상 포워딩으로 신뢰를 위장하는 경우
공격자를 오히려 ``좋은 부모''로 오래 유지할 수 있다.
본 논문은 Backpressure RPL(BRPL)의 부모 선택 점수에
경량 신뢰 패널티를 직접 결합한 \textbf{TA-BRPL}을 제안한다.
TA-BRPL은 전달 신뢰도($T_{fwd}$), 제어 평면 일관성($T_{ctrl}$),
큐 정직성($T_{hon}$)의 세 신호를 가중 기하 평균으로 집계하고,
비대칭 EWMA, 2단계 review/blacklist, admission/dwell/switch-margin 게이트를 통해
신뢰를 \emph{탐지 알람}이 아니라 \emph{라우팅 의사결정}에 안정적으로 결합한다.
...
\begin{minipage}{0.96\textwidth}
\enabstract{%
In low-power and lossy networks (LLNs), RPL must balance low control overhead,
topology stability, and resilience to routing manipulation.
This balance becomes fragile under sinkhole attacks:
an attacker first forges an attractive low-rank advertisement to capture routes,
then selectively drops traffic to degrade availability.
Packet-delivery-ratio (PDR)-centric evaluation misses this early route-capture phase,
whereas aggressive trust-based eviction often destabilizes routing through churn explosion.
Queue-aware protocols such as Backpressure RPL (BRPL) improve congestion handling,
yet remain vulnerable when an attacker lies about queue state or behaves benignly long enough
to retain parent preference.
...
TA-BRPL combines three trust axes---forwarding fidelity ($T_{fwd}$),
control-plane consistency ($T_{ctrl}$), and queue-advertisement honesty ($T_{hon}$)---%
through a weighted geometric mean, then couples the resulting trust to routing
via asymmetric EWMA, two-stage review/blacklist validation, and stabilization gates
(admission, dwell time, and switch margin).
...
by $\Delta = -0.0169$ ($p < 10^{-4}$, 76\% topology win-rate),
reduces hit ratio by $\Delta = -0.0166$,
improves attack-phase PDR by $\Delta = +0.0102$ ($p = 0.002$),
and limits churn overhead to $\Delta = +0.0364$, well below the pre-declared bound of $+0.1$.
These findings suggest that the key challenge in LLN secure routing is not only detecting
...
공격자는 selective forwarding이나 grayhole 동작으로
가용성을 크게 훼손할 수 있다.
...
BRPL의 queue term은 오히려 공격자를 ``좋은 부모''로 오래 유지할 수 있다.
즉 BRPL류는 혼잡에 강하지만,
...
\textbf{selective forwarding / grayhole}은 일정 비율만 드롭해
은닉성을 높이는 경우다.
...
정규화 신뢰 $t(v)=\hat{T}(v)/1000$, 불신뢰도 $d(v)=1-t(v)$에 대해
BRPL 점수 반영식은:
\begin{equation}
...
허용 임계는 충분히 검증되지 않은 후보가
``새로운 좋은 부모''처럼 갑자기 들어오는 것을 제한한다.
...
v13.12(최종)에서 +0.036으로 감소한 추이를 확인할 수 있다.
...
본 연구 결과는 유의미하지만, 몇 가지 중요한 한계를 함께 해석해야 한다.
...
특히 저밀도 네트워크에서는 귀책 편향이 완전히 사라지지 않는다.

\textbf{정적 시뮬레이션 환경.}
실험은 Contiki-NG/Cooja와 UDGM 무선 모델에서 수행되었다.
이는 재현성과 통제성 측면에서는 장점이 있으나,
...
동급 보안 라우팅과의 직접 비교도 후속 과제로 남아 있다.
...
공통 파라미터가 모든 밀도에 동일하게 최적은 아니라는 점을 시사한다.
...
핵심 결론은 세 축으로 정리된다.
\textbf{(i)~경로 장악 억제}: att\_share/hit\_ratio가 유의하게 감소했고 ($p<10^{-4}$),
이는 공격자 의존 강도와 오염 범위를 동시에 줄였음을 뜻한다.
\textbf{(ii)~가용성 보전}: 공격 구간 PDR은 평균적으로 개선되었고,
토폴로지의 93.3\%가 비열세 기준을 통과했다.
\textbf{(iii)~안정성 상한}: churn 증가는 존재하지만
95\% CI 상한이 사전 선언한 $+0.1$보다 충분히 낮아,
...
향후에는 stage-split 인과 검증, 다중 공격자, 현실 무선 모델,
에너지/지연 오버헤드, 그리고 RFTrust/PCC-RPL 같은
동급 보안 라우팅과의 직접 비교를 통해
TA-BRPL의 적용 범위와 일반화 가능성을 더 엄밀히 검증할 계획이다.
