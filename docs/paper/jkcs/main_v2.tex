...
노드 내 암호화 없는 신뢰 패널티를 직접 결합한 \textbf{TA-BRPL}을 제안한다.
...
We propose \textbf{TA-BRPL}, a trust-aware extension that embeds an on-node, cryptography-free trust penalty
...
\begin{table*}[!t]
\centering\small
\caption{RPL 보안 방어 계열의 문헌 기반 정성 위치 비교
  (qualitative positioning; O: 충족, P: 부분, X: 미충족).
  표의 목적은 설계 축에서의 상대적 위치를 요약하는 것이며,
  RFTrust/PCC-RPL 등과의 실험적 성능 비교를 의미하지 않는다.}
\label{tab:relwork}
\resizebox{\linewidth}{!}{%
...
\noindent
표~\ref{tab:relwork}는 기존 연구와 본 연구의 설계적 위치를
문헌 기반으로 요약한 것이다.
...
이는 이론적으로 도출한 최적 가중치가 아니라,
신호의 관측성 및 공격 특이성에 대한 직관과 반복 실험을 통해 정한
경험적 operating point이다.
추가로 그림~\ref{fig:weight_sensitivity}는
9쌍 miniset에서 현재 operating point 주변의 10개 국소 조합
(현재값 포함)을 대상으로 한 탐색적 sensitivity scan을 보여준다.
증거 수준은 75쌍 메인 실험과 동일하지 않으므로,
이 그림을 가중치 최적성이나 일반화의 근거로 해석해서는 안 된다.
다만 10개 조합 모두에서 평균 $\Delta$att\_share가 음수였고
범위도 $-0.0174$에서 $-0.0204$로 비교적 좁아,
현재값 $(0.5,0.3,0.2)$가 주변 조합 대비
비정상적으로 날카로운 특이점처럼 보이지는 않음을
보조적으로 확인할 수 있다.
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=\columnwidth]{fig_jkcs_weight_sensitivity_heatmap.pdf}
\caption{가중치 sensitivity heatmap
  (9쌍 miniset의 탐색적 보조 분석; 총 10개 조합이며 별표는 현재값).
  현재값 $(0.5,0.3,0.2)$는 주변 조합과 유사한 $\Delta$att\_share를 보여
  국소적으로 특이한 한 점으로 보이지 않는다.}
...
이 구조는 route capture 완화와 stability 보존을 동시에 겨냥한 핵심 설계 특징이다.
...
또한 그림~\ref{fig:churn_att_scatter}는 각 토폴로지 쌍에서
$\Delta$att\_share와 $\Delta$churn의 관계를 직접 보여준다.
즉 $\Delta$att\_share가 개선된 케이스에서
$\Delta$churn이 비례해서 증가하는 경향이 관찰되지는 않으며,
TA-BRPL의 att\_share 개선이 단순히 과도한 부모 전환을 대가로 얻어진 결과는
아님을 시사한다.
다만 75개 쌍 중 12개는 개별 토폴로지 수준에서
$\Delta$churn $> +0.1$을 보였고, 이 outlier는 대부분 저밀도(sparse)에 집중된다.
따라서 H3는 모든 토폴로지에 대한 hard cap이 아니라,
쌍체 평균과 신뢰구간에 대한 안정성 가설로 해석해야 한다.
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=\columnwidth]{fig_jkcs_churn_att_scatter.pdf}
\caption{토폴로지별 $\Delta$att\_share--$\Delta$churn 산점도.
  63/75개는 H3 상한($\Delta$churn $\le +0.1$) 안에 머물고
  12개 outlier는 주로 sparse에 나타난다.
  그럼에도,
  격리 개선이 단순한 churn 희생의 결과가 아님을 보여준다.}
\label{fig:churn_att_scatter}
\end{figure}
...
그림~\ref{fig:ablation_forest}는 이 결과를 시각적으로 요약한다.
즉, $T_{fwd}$ 또는 $T_{fwd}{+}T_{ctrl}$만으로는 안정적 격리 개선이 나타나지 않고,
세 신호를 함께 결합한 Full 모델에서만
$\Delta$att\_share의 신뢰구간이 일관되게 0 아래로 내려간다.

\begin{figure}[!tbp]
\centering
\includegraphics[width=\columnwidth]{fig_jkcs_ablation_forest.pdf}
\caption{Ablation의 $\Delta$att\_share 및 95\% CI.
  Full 모델만 신뢰구간이 완전히 0 아래에 위치한다.}
...
그림~\ref{fig:trust_timeseries}는 대표 사례에서 공격자 이웃에 대한
$T_{fwd}$, $T_{ctrl}$, $T_{hon}$, $T_{agg}$의 시계열을 보여준다.
대표 사례는 75쌍 중 $\Delta$att\_share$<0$ 및 $\Delta$churn$\le +0.1$을 만족하는 후보를 먼저 추린 뒤,
공격 구간에서 $T_{agg}$ 하락이 관찰되는 경우만 남기고,
그중 $\Delta$att\_share가 후보군 중앙값에 가장 가까운 토폴로지를 선택했다.
$T_{ctrl}$은 초기 유인(rank lure)에 빠르게 반응하고,
$T_{fwd}$는 공격 구간이 진행되며 누적 관측에 따라 하락하며,
최종적으로 $T_{agg}$가 낮아지면서 공격자 부모의 선호도가 감소한다.
즉, trust 파이프라인은 정적인 판정기가 아니라
시간에 따라 축적된 증거를 라우팅 비용으로 환원하는 방식으로 동작한다.
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=\columnwidth]{fig_jkcs_trust_timeseries.pdf}
\caption{선정 규칙을 명시한 대표 사례에서 공격자 이웃 trust 신호의 시계열 변화.
  점선은 공격 시작(350s)과 종료(650s)를 나타낸다.
  $T_{ctrl}$의 조기 반응과 $T_{fwd}$의 후행 하락이 함께 누적되며
  $T_{agg}$가 감소한다.}
\label{fig:trust_timeseries}
\end{figure}
...
또한 고정 토폴로지 기반 튜닝과 랜덤 토폴로지 평가 사이에
엄격한 held-out 분리가 완전히 보장된 것은 아니다.
따라서 본 결과는 메커니즘의 재현 가능성을 보여주지만,
model-selection leakage와 optimistic bias 가능성을 완전히 배제하지는 않는다.

\textbf{혼잡 보존 주장 미검증.}
본 실험에는 혼잡 유도 workload가 포함되지만,
혼잡-only, 공격-only, 혼잡+공격을 분리한 직접 비교는 수행하지 않았다.
따라서 현재 데이터가 직접 지지하는 주장은
``혼잡 유도 구간을 포함한 공격 시나리오에서 attack-phase PDR 비열세를 보였다''는 수준이며,
BRPL의 혼잡 이점을 완전히 보존했다고 단정할 수는 없다.
...
TA-BRPL의 상대적 특성과 적용 범위를 더 엄밀히 규명할 필요가 있다.
이는 다음 revision의 우선 과제(F1)로 설정할 계획이다.
